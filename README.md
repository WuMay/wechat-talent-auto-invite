...
[分支判断]
  ...
  │                 ↓
  │               返回达人广场 → 从下一个达人继续（记录Y坐标避免重复）
  │
  └─ 未找到邀约按钮 → Ctrl+W 关闭窗口 → 跳过该达人 → 从下一个达人继续
                      （该达人可能已邀约或不支持邀约）
```

### ✅ 避免重复处理机制
脚本使用"位置记忆"机制避免重复处理同一个达人：

1. **记录点击位置**：每次点击"详情"按钮后，记录Y坐标（`lastClickY`）
2. **限制搜索范围**：下次搜索"详情"按钮时，从 `lastClickY + 20` 像素开始搜索
3. **避免重复**：确保每次只处理下一个新的达人，不会重复点击同一个

**原理图**：
```
达人列表页面：
┌─────────────────────┐
│ 达人1 详情按钮 ← 第一次点击这里，记录Y坐标
│ 达人2 详情按钮 ← 第二次从Y+20开始搜索，找到这里
│ 达人3 详情按钮 ← 第三次继续往下搜索
│ ...
└─────────────────────┘
```
...
### 5. images 目录找不到或图像文件缺失
**报错示例**：
- "未找到 images/ 目录"
- "缺少以下图像文件"

**调试步骤**：
1. 查看报错提示中显示的实际路径（例如：`C:\Users\xxx\Desktop\脚本\images`）
2. 检查这个路径是否真的存在
3. 如果是相对路径问题：
   - 确保脚本文件和 images 文件夹在**同一级目录**
   - 不要把 images 文件夹放在子目录中

**正确的目录结构**：
```
某个文件夹/
├── wechat_talent_v2_image.ahk  ← 脚本文件
└── images/                      ← images 文件夹（必须在这里）
    ├── detail_button.png
    ├── invite_button.png
    └── ...
```
...
**验证方法**：
- Windows：右键点击 images 文件夹 → 属性 → 查看位置
- 确保脚本和 images 文件夹在同一个位置
